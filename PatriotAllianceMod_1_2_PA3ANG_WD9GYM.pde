/*******************************************************************************
    Modifications by WD9GYM
    
**** Note when using the Morse code decoder RIT might have to be at -1 to -3 ***

1) Change to I2C LCD.
2) Change Startup to include version and date
   and my id 
3) Add   Splash_MODE(); to setup after version display
4) Move frequency and rit to top line remove --- lines from setup
   display rit even if zero.
5) Move Band Width, Step, Mode, Key Type and Smeter to line 2
6) Add battery display and old band display to line 3
   Add battery volatiles below Smeter's
   Add old txt 66 & 67 variables
   Add battery stuff in setup and spalsh mode  W8CQD
   Add call to Battery Splash in loop
   Add splash functions for band and batery at end of source
   Add PAM version to line 4 to alternate with decoder output
7) Add decoder variables   
   Add decoder setup
   Add flip code int Selection()
   Add decoder support routines at end of source
   Add #ifdef for decoder or announce in selection()
8) Change 40 M band limit   
9) change 20 M startup freq
10) remove T # # on transmit key
11) set selected_step = 0
12) add wpm display on line 2
13) add .005 to code analog read
14) always display rit offset
15) change frequency display to add one more significant digit which moves
    with rit knob. Gives operator finer control without changing values.
    See splash_RX_freq() lcd.print statements
16) Change step select initial value to 10 Hz    
17) change compile date
18) Change 40M upper band freq 
19) Define switch pin and set LOW
20) move decoder boolean up here
21) remove decoder code
22) add change state of switchPin in flip routine
23) try reading serial 1 for decoded char worked
********************************************************************************/
#define DECODER 1
#define switchPin 35 // wont; work with Adafruit
boolean enableDecoder = false; // decoder is off
...
const long meter_20             = 14047000;     // ENTER YOUR STARUP FREQUENCY HERE FOR 20 MTR IN Hz    W1AW Code practice
...
void setup() 
{
  ...
  lcd.setCursor(0,0);
 lcd.print("WD9GYM/PA3ANG ");
  lcd.setCursor(0, 1);
  lcd.print("05/08/2015 PAM 1.2");
  ...
  #endif
  delay(2000);
  lcd.clear();         // erase version display
...
void splash_RX_freq()
{
  ...
  if ( old_frequency_tune != frequency_tune || forcedSplash )
  {
    ...
    {
    lcd.print("14." + stringFREQ.substring(2,5) + "."+ stringFREQ.substring(5,9));
    }
    else  
    {
    lcd.print("07." + stringFREQ.substring(1,4) + "."+ stringFREQ.substring(4,8));
    }  
    #ifdef OLED
...
void writeSmeter()                 // WD9GYM modification - adjusted by W8CQD
{
    ...
    if (smeterInterval > 200) // ignore checks less than 200mS after initial edge
  {
    ...
    lcd.setCursor(14, 1);                      // print the s meter on line 4
